package com.felix.magneticpatchimageview.view;
...
/**
 * Created by Felix.Zhong on 2018/6/19 22:45
 * 磁贴效果
 */
public class MagneticPatchImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public MagneticPatchImageView(Context context) {
        super(context);
        camera = new Camera();
    }

    public MagneticPatchImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        camera = new Camera();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                float x = event.getX();
                float y = event.getY();
                isActionMove = x > vWidth || y > vHeight || x < 0 || y < 0;
                break;
